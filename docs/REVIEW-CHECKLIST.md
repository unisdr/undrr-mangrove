...
- [ ] Class elements that render as visual headings (e.g. `__title`, `__heading`, `__value`, large nav labels) use `$mg-font-family-headings`, not `$mg-font-family-condensed` directly. Add a matching `:lang(ar)` block routing to `$mg-font-family-arabic-headings`, since Roboto Condensed has no Arabic glyph coverage.
