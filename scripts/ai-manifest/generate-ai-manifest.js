...
The Storybook includes a Brand section for non-technical users (content editors, brand managers, external partners). Thirteen pages:

- **Where to start** (${DOCS_BASE}?path=/docs/brand-where-to-start--docs): Canonical reading order, skip-to paths per audience (designer / writer / developer / external partner), and a five-minute path for the irreducible core. Read this first.
...
- **Written voice** (${DOCS_BASE}?path=/docs/brand-written-voice--docs): Four voice registers (Urgent, Authoritative, Warm, Hopeful) mapped to content type, plus house conventions: British English (Oxford spelling), sentence case, em-dashes in body prose only (never in UI), descriptive link text, gender-neutral and disability-inclusive language.
...
- **Sub-brand rationale** (${DOCS_BASE}?path=/docs/brand-sub-brand-rationale--docs): Why each sub-brand (PreventionWeb, MCR2030, IRP, ARISE, DELTA Resilience) looks and sounds different — audience, register skew (working hypothesis), visual emphasis, sub-brand-specific anti-patterns. The thread (typography, voice trio, conventions) is constant.
- **Anti-patterns** (${DOCS_BASE}?path=/docs/brand-anti-patterns--docs): Explicit list of what UNDRR is *not* — conference rooms, disaster tourism, generic blue-NGO aesthetic, jargon walls, empty optimism, gendered language, em-dashes in UI, etc., each with counter-examples. Pre-publish checklist grouped by stakes (must hold / should hold / craft).
- **AI imagery prompts** (${DOCS_BASE}?path=/docs/brand-ai-imagery-prompts--docs): Copy-paste prompt templates for image and language models, with explicit standing rules: no photorealistic AI of human beings (only stylised illustration); no AI imagery for named people or specific real events; no AI-generated children; named country (not region); disability inclusion required; decolonial framing required. Derivative of UNDRR institutional AI guidance, which is in development.
