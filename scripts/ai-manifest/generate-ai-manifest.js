...
- **Layered identity** (${DOCS_BASE}?path=/docs/brand-layered-identity--docs): The strategic model behind UNDRR's identity — core UNDRR → sub-brand (PreventionWeb, MCR2030, IRP, ARISE, DELTA) → campaign / partner product. At every layer boundary, names the *thread* (constant) and the *variable* (allowed to flex). Canonical home for the "what threads through every layer" table. Includes a worked IDDRR campaign example.
...
- **Sub-brand rationale** (${DOCS_BASE}?path=/docs/brand-sub-brand-rationale--docs): Why each sub-brand (PreventionWeb, MCR2030, IRP, ARISE, DELTA Resilience) looks and sounds different — audience, register skew (working hypothesis), visual emphasis, sub-brand-specific anti-patterns, and the per-sub-brand AI imagery prompt variant (colour swap + bolt-on cues).
