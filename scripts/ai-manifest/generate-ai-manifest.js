...
The Brand section is the canonical reference for UNDRR web identity. It lives inside the Mangrove component library because the truth ships in the code, but it is written for designers, content editors, brand managers, and external partners — not just developers. Treat it as the north-star reference: when a guideline conflicts with another source (older SharePoint pages, legacy PDFs), the Brand section wins for web work. Each fact has one canonical home; other pages summarise and link rather than duplicate. Fourteen pages:

- **Where to start** (${DOCS_BASE}?path=/docs/brand-where-to-start--docs): Canonical reading order, skip-to paths per audience (designer / writer / site editor producing AI imagery / developer / external partner), and a five-minute path for the irreducible core. Read this first.
...
- **Brand guidelines** (${DOCS_BASE}?path=/docs/brand-brand-guidelines--docs): Editorial guidance — brand positioning (K/A/C trio canonical home), communication rules, logo usage, photography standards (including the canonical photo size table), web elements. Migrated from UNDRR SharePoint.
- **Layered identity** (${DOCS_BASE}?path=/docs/brand-layered-identity--docs): The strategic model behind UNDRR's identity — core UNDRR → sub-brand (PreventionWeb, MCR2030, IRP, ARISE, DELTA) → campaign / partner product, plus Layer 2.5 for the five regional offices. At every layer boundary, names the *thread* (constant) and the *variable* (allowed to flex). Canonical home for the "what threads through every layer" table, the one-row-per-sub-brand summary (audience / register / primary accent / distinctive emphasis), and the five-question sub-brand decision rule. Includes a worked IDDRR campaign example. Self-sufficient for scoping decisions; sub-brand rationale page is the deeper reference.
- **Visual voice** (${DOCS_BASE}?path=/docs/brand-visual-voice--docs): How color, typography, photography, iconography, layout rhythm, motion, illustration, and data viz express the K/A/C positioning trio (Knowledgeable, Approachable, Collaborative). Practical companion to brand guidelines; references the canonical tables rather than duplicating them.
- **Written voice** (${DOCS_BASE}?path=/docs/brand-written-voice--docs): Six observed voice registers — Authoritative, Procedural (Words into Action voice; numbered, defined-terms, prescriptive), Warm, Hopeful (quietly evidence-based, not campaign optimism), Diplomatic (commemorative observances; "championed by [Member State]"; multilateral attribution), and Time-sensitive (reframed from "Urgent"; UNDRR rarely voices live-disaster content itself, this register is for early-warning calls-to-action, EW4All milestones, and partner-amplification on behalf of OCHA / IFRC / national authorities). Marked as observed framework synthesised from sampling UNDRR output, pending UNDRR Communications confirmation. Plus house conventions: British English (Oxford spelling), sentence case, em-dashes in body prose only (never in UI), descriptive link text, gender-neutral and disability-inclusive language. Common rewrites table for register drift.
- **Audience mapping** (${DOCS_BASE}?path=/docs/brand-audience-mapping--docs): How voice and visual emphasis shift across UNDRR's primary web audiences. Eleven-row primary-audience table covering governments and policy-makers, practitioners, science and technology community (STAG), local governments, private sector (SMEs and member companies), civil society and NGOs, parliamentarians, UN system and Resident Coordinators, media (intermediaries), general public via campaigns, and partners and donors. Cross-cutting inclusion-audiences section (women, children and youth, persons with disabilities, older persons, Indigenous Peoples, migrants — all named in the Sendai Framework with dedicated UNDRR programmes). One-story-multiple-framings worked example (pedagogical; pointers to real worked cases via the MCR2030 Pathways to Resilience publication). Framed as Mangrove-side web-framing aid, not as a complete UNDRR stakeholder model — see the UNDRR Partners and stakeholders page for the canonical institutional list.
- **Sub-brand rationale** (${DOCS_BASE}?path=/docs/brand-sub-brand-rationale--docs): Layer 2 in depth — for the moments when you are actively producing for a specific sub-brand. Per-sub-brand visual emphasis, sub-brand-specific anti-patterns, and the AI imagery prompt variant (colour swap + bolt-on cues). The summary table and decision rule live on the Layered identity page; this page is the deeper reference once the scoping is settled.
- **Anti-patterns and pre-publish checklist** (${DOCS_BASE}?path=/docs/brand-anti-patterns--docs): The unified pre-publish checklist for any UNDRR artifact, grouped by stakes (Must hold / Should hold / Craft). Plus a short list of anti-patterns worth naming explicitly (Sendai stripe wallpaper, theme leakage, conference-photo aesthetic in illustration, decorative-only Sendai logo, mixed icon systems, Font Awesome on new properties, campaign without sub-brand thread, sub-brand without UN-family thread). Every other page links here for the checklist.
- **AI imagery policy** (${DOCS_BASE}?path=/docs/brand-ai-imagery-policy--docs): The 12 standing rules for generative imagery (no photorealistic AI of human beings; no AI imagery for named people or specific real events; no identifiable human faces; no children; no rendered text or icons in AI outputs; named country not region; diversity-within-named-place rule; disability inclusion required as peer; decolonial framing required; specific hex codes for colours; label AI-generated imagery on publication). Institutional sources: UN System AI Principles (CEB 2022), UNESCO AI Ethics Recommendation (2021), UN System AI Governance White Paper (HLCP 2024), UNDRR Communications guidance (in development). Read before generating anything.
- **AI imagery prompts** (${DOCS_BASE}?path=/docs/brand-ai-imagery-prompts--docs): The workflow page. Officers do not write the image prompt themselves — a language model (Gemini / Claude / ChatGPT) writes it, given (a) the officer's one-sentence conceptual brief (e.g. "I want to show how community-led DRR practices protect neighbourhood forests and coastal communities") and (b) the UNDRR brand block on this page. The LLM applies the 13 standing rules from the AI imagery policy and asks up to three clarifying questions if the brief is thin. Result paste into Midjourney / DALL-E / Imagen / Stable Diffusion. Includes a worked end-to-end example (brief → clarifying questions → finished prompt → next step), the sub-brand colour-swap and cues table, four reference example prompts for when you want to see finished output without going through the LLM, and three language-model templates for editorial copy alongside imagery (alt text, British-English converter, sentence-case rewriter). Full register-rewriter prompt lives on the Written voice page.
- **Common patterns** (${DOCS_BASE}?path=/docs/brand-common-patterns--docs): Plain-language reference for foundations shared across all themes — breakpoints, grid classes (\`mg-grid__col-{N}\`), accessibility standards, language support (\`lang="ar"\` mechanism), icon sources, z-index. Canonical home for the typography family table.
- **Component gallery** (${DOCS_BASE}?path=/docs/brand-component-gallery--docs): Decision guidance for the moments when multiple components could plausibly do the job — which hero (Hero / Child hero / Feature promo), which card (Vertical / Horizontal / Book / Icon / Stats), which way to call out content (Quote highlight / Highlight box / DocCallout), Pager vs Pagination, in-page navigation (Mega menu / Breadcrumbs / On-this-page nav / Table of contents), Buttons vs Chips, data viz by data type. Plus the three required-on-every-page platform scripts: Analytics enhancements (GA4 + per-domain property configuration), Critical messaging (failsafe broadcast that works even when the CMS is down), and Cookie consent (shared CDN bundle; React-component option for React integrations). Sits on top of the Storybook sidebar (which is the full catalog); does not re-list every component. Points to the sidebar for layout utilities, snackbar, syndication, fetchers, and the page template.
