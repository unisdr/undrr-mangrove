...
- **Consider auto-rendering.** If your component renders cleanly in Node.js (no browser APIs), add a webpack entry in `webpack.config.js` and sample props in `SAMPLE_PROPS` in `scripts/ai-manifest/render-component-html.js`. Auto-rendered HTML stays in sync automatically and requires no manual maintenance.
