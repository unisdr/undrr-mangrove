...
- **Consider auto-rendering.** If your component renders cleanly in Node.js (no browser APIs, or only DOMPurify which is mocked), add it to `RENDER_SPECS` in `scripts/ai-manifest/render-component-html.js`. Auto-rendered HTML stays in sync automatically and requires no manual maintenance.
