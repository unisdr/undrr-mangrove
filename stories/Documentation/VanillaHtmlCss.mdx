...
**Important:** React components require **same-origin hosting**. Import maps only apply to modules loaded from your domain, not cross-origin CDNs. For external demos (CodePen, etc.), use vanilla HTML components instead.

**Same-origin deployment (Drupal, self-hosted):**

React 19 dropped UMD builds, so we use [import maps](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/script/type/importmap) with [esm.sh](https://esm.sh/):
...
<body>
  ...
  <!-- Component must be served from same origin -->
  <script type="module">
    import React from 'react';
    import { createRoot } from 'react-dom/client';

    // Load component from same-origin path (copy from dist/components/)
    const MegaMenuModule = await import('/js/mangrove/MegaMenu.js');
    const MegaMenu = MegaMenuModule?.default ?? MegaMenuModule;

    const root = createRoot(document.getElementById('mega-menu-root'));
    root.render(
      React.createElement(MegaMenu, {
        sections: [
          { title: 'About', items: [{ title: 'Our Work', url: '/about' }] }
        ]
      })
    );
  </script>
...
[See vanilla HTML example on CodePen](https://codepen.io/khawkins98/pen/MYwbKwe)
