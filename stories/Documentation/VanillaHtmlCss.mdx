...
Components that support hydration (ShareButtons, ScrollContainer) export a `fromElement` function that extracts props from data attributes. Combined with the `createHydrator` helper, this eliminates all manual `createRoot` boilerplate:
...
Components that don't yet export `fromElement` (MegaMenu, BarChart, MapComponent, etc.) can be mounted manually with `createRoot`:
