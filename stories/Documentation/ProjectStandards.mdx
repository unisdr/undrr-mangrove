...
UNDRR experienced firsthand how design fragmentation led to inefficiencies and rising maintenance costs. Previously, each platform (PreventionWeb, Sendai Framework Monitor, and others) maintained its own UI system, multiplying effort across accessibility, rebranding, and onboarding tasks.
...
Mangrove is the single source of truth for the UNDRR digital experience. This means:
...
- Centralized updates that reach all properties at once
...
.mg-cta-button--report-download {
  border-left: 4px solid $mg-color-accent-100;
}
...
function ReportSummary({ report }) {
  return (
    <>
      <QuoteHighlight
        quote={report.pullQuote}
        attribution={report.author}
        variant="line"
      />
      <StatsCard
        title={report.title}
        stats={report.keyStats}
      />
    </>
  );
}
...
.mg-mega-menu__user-section {
  ...
  .mg-button--user-profile {
    ...
    &:hover {
      background: $mg-color-neutral-25;
    }
  }
}
```
...
```jsx
import { StatsCard, ShareButtons } from '@undrr/undrr-mangrove';
```

### Sass

Import the Mangrove design tokens into your SCSS build. The path depends on your setup:

```scss
// Within the Mangrove repo
@import "stories/assets/scss/variables";

// From an npm consumer (path depends on your bundler's resolve config)
@import "node_modules/@undrr/undrr-mangrove/stories/assets/scss/variables";
```

See the <LinkTo kind="getting-started-sass-integration" story="docs">Sass integration guide</LinkTo> for full setup instructions.
...
Mangrove is how UNDRR ships consistent, accessible web experiences. Use it, extend it, improve it -- just don’t reinvent it.

**Next steps:**

- <LinkTo kind="getting-started-a-getting-started-guide" story="docs">Getting started guide</LinkTo>
- <LinkTo kind="getting-started-component-contribution-guide" story="docs">Component contribution guide</LinkTo>
