...
import { TextInput } from '@undrr/undrr-mangrove/components/TextInput';
...
```jsx
<TextInput
  type="text"       // "text" | "email" | "password" | "number" | "tel" | "search" | "date" | "url"
  label="Field label"     // Visible label text
  hideLabel={false}        // Visually hide label (still accessible)
  placeholder="Enter text..." // Placeholder text
  helpText="Help text"    // Help text below the input
  error={false}            // Error state
  errorText="Error message" // Error message (shown when error is true)
  id="field-id"            // Custom id (auto-generated if omitted)
...
<Textarea
  ...
  helpText="Max 500 characters" // Help text
  error={false}            // Error state
  errorText="Required"    // Error message
  id="textarea-id"        // Custom id
...
- **Server-rendered hydration**: For Drupal, Astro, or other server-rendered frameworks, the [Hydration guide](https://github.com/unisdr/undrr-mangrove/blob/main/docs/HYDRATION.md) shows how to render React components into server-generated HTML containers using `createHydrator`
