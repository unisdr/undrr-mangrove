...
{/* prettier-ignore */}
For the canonical reading order across the Brand section, see <LinkTo kind="brand-where-to-start" story="docs">Where to start</LinkTo>. For the strategic model behind these guidelines (core UNDRR → sub-brand → campaign), see <LinkTo kind="brand-layered-identity" story="docs">Layered identity</LinkTo>. For practical voice and tone direction, see <LinkTo kind="brand-visual-voice" story="docs">Visual voice</LinkTo>, <LinkTo kind="brand-written-voice" story="docs">Written voice</LinkTo>, <LinkTo kind="brand-audience-mapping" story="docs">Audience mapping</LinkTo>, <LinkTo kind="brand-sub-brand-rationale" story="docs">Sub-brand rationale</LinkTo>, and <LinkTo kind="brand-anti-patterns" story="docs">Anti-patterns and pre-publish checklist</LinkTo>. For generative imagery, see the <LinkTo kind="brand-ai-imagery-policy" story="docs">AI imagery policy</LinkTo> (the standing rules) and the <LinkTo kind="brand-ai-imagery-prompts" story="docs">AI imagery prompts</LinkTo> cookbook (formula plus an LLM meta-prompt).
...
> are increasing. However, it is not so much about the organisation but more
> about the agenda."
>
> – Mami Mizutori, former SRSG for Disaster Risk Reduction (2018–2024)
...
- Be inclusive and promote other players (this is part of the organisation's mandate), but claim ownership when appropriate
- Increase social media presence and communicate rapidly when disasters occur (with emphasis on prevention and recovery)
- Harmonise digital design around Sendai branding (colourful and recognisable)
...
The [Sendai Framework for Disaster Risk Reduction 2015–2030](https://www.undrr.org/publication/sendai-framework-disaster-risk-reduction-2015-2030) (Sendai Framework) is the international agreement UNDRR coordinates implementation of; its logo is institutional, not decorative.
...
- The Sendai Framework logo can be used to create sub-brands. The sub-brand should appear above the colour stripe to separate it from the overall concept.

### UN and SDG logos

- When using the UN logo, it takes precedence over UNDRR.
- UNDRR can be used in conjunction with the Sustainable Development Goals (SDG) logo and wheel.
...
The page header displays the Sendai colour stripe, UNDRR logo, and global navigation
...
The footer is common across all UNDRR sites in appearance, with customisable link sections:
...
All external-facing print and web material should use **Roboto**; headings use **Roboto Condensed**. These are the UNDRR-adopted web typefaces, consistent with UN visual identity practice; the "UN official typeface" attribution is sourced from internal UNDRR Communications documentation and pending verification against the public [UN Web Standards](https://www.un.org/styleguide/). For internal communication and email, Arial can be used as a substitute.

**Language-specific fonts:** Mangrove supports automatic font overrides for scripts that
require specialised typography. When content is marked with a language attribute
...
> **Note:** The older UNDRR SharePoint Colors & Typography page lists Noto Nasteh for Arabic. Mangrove supersedes that – the live source of truth for web is the table above. If you find conflicting guidance elsewhere, default to Mangrove and flag the source for the UNDRR Communications team to update.
