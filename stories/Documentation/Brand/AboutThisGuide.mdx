import { Meta } from '@storybook/addon-docs/blocks';
import LinkTo from '@storybook/addon-links/react';
import { StorybookNavCard } from './components/StorybookNavCard';
import { DocCallout } from './components/DocCallout';

<Meta
  title="Brand/About this guide"
  parameters={{
    viewMode: 'docs',
    previewTabs: {
      canvas: { hidden: true },
    },
  }}
/>

# UNDRR web style guide

<DocCallout variant="warning">
  <strong>Prototype guidance, under review (May 2026).</strong> The Brand section is a working draft pending UNDRR Communications confirmation. Treat its claims as provisional and contribute corrections via <a href="https://github.com/unisdr/undrr-mangrove/issues">a GitHub issue</a> or <code>undrrcomms@un.org</code>.
</DocCallout>

Colours, fonts, and components used across UNDRR websites.

---

## Who this is for

{/* prettier-ignore */}
For content editors, designers, and partners who work with UNDRR web properties. This guide shows the visual identity of each property so you can stay on-brand without needing to read code or install anything. For developers, see <LinkTo kind="getting-started-getting-started-guide" story="docs">Getting started</LinkTo>.

---

## Choose your property

{/* prettier-ignore */}
Each UNDRR web property has its own visual identity. Click a card to see that property's colours, fonts, and component styles on the <LinkTo kind="brand-brand-identity" story="docs">Brand identity</LinkTo> page.

<div
  style={{
    display: 'grid',
    gridTemplateColumns: 'repeat(3, 1fr)',
    gap: '1rem',
  }}
>
  <StorybookNavCard
    title="UNDRR"
    summary="undrr.org"
    theme="Global UNDRR Theme"
    imgback="/img/brand-guides/undrr.svg"
    imgalt="UNDRR brand card"
  />
  <StorybookNavCard
    title="PreventionWeb"
    summary="preventionweb.net"
    theme="PreventionWeb Theme"
    imgback="/img/brand-guides/preventionweb.svg"
    imgalt="PreventionWeb brand card"
  />
  <StorybookNavCard
    title="MCR2030"
    summary="mcr2030.undrr.org"
    theme="MCR2030 Theme"
    imgback="/img/brand-guides/mcr2030.svg"
    imgalt="MCR2030 brand card"
  />
  <StorybookNavCard
    title="IRP"
    summary="International Recovery Platform"
    theme="IRP Theme"
    imgback="/img/brand-guides/irp.svg"
    imgalt="IRP brand card"
  />
  <StorybookNavCard
    title="DELTA Resilience"
    summary="deltaresilience.org"
    theme="DELTA Resilience Theme"
    imgback="/img/brand-guides/delta.svg"
    imgalt="DELTA Resilience brand card"
  />
  <StorybookNavCard
    title="ARISE"
    summary="ariseglobalnetwork.org"
    theme="ARISE Theme"
    imgback="/img/brand-guides/arise.svg"
    imgalt="ARISE brand card"
  />
</div>

---

## How to use the theme switcher

Use the **theme selector** in the toolbar above to see how components look in each
property's style. When browsing a brand page, switch to that property's theme for
the most accurate preview.

---

## Quick links

{/* prettier-ignore */}
- <LinkTo kind="getting-started-getting-started-guide" story="docs">Getting started guide</LinkTo> (for developers)
- <LinkTo kind="design-decisions-colors" story="docs">Full colour reference</LinkTo> (all tokens, developer-oriented)
- <LinkTo kind="design-decisions-typography" story="docs">Full typography reference</LinkTo>
- [Report an issue](https://github.com/unisdr/undrr-mangrove/issues)

---

Last updated: 18 May 2026
