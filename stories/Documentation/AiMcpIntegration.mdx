...
The [utilities reference](https://unisdr.github.io/undrr-mangrove/ai-components/utilities.json) (~25 KB) lists all 161 CSS utility classes grouped into 12 categories. Each class has a description and usage example.
...
`render-component-html.js` imports built components from `dist/` and renders them with sample props using `renderToStaticMarkup`. Components that render cleanly in Node.js get auto-generated HTML. The script mocks DOMPurify so components that use it for HTML sanitization (e.g., IconCard) can also be auto-rendered. Components needing other browser APIs (`document`, `window`) fall back to curated HTML examples.
...
- **Expand auto-rendering** — IconCard is now auto-rendered via a DOMPurify mock, joining QuoteHighlight, StatsCard, Pager, MegaMenu, and SyndicationSearchWidget. Components that depend on `document` or `window` (e.g., ShareButtons) still need curated HTML
