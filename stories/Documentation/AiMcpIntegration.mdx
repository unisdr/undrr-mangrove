...
- **CSS utility inventory** with all 162 utility classes grouped by category.
...
The [utilities reference](https://unisdr.github.io/undrr-mangrove/ai-components/utilities.json) (~25 KB) lists all 162 CSS utility classes grouped into 12 categories. Each class has a description and usage example.
...
`render-component-html.js` imports built components from `dist/` and renders them with sample props using `renderToStaticMarkup`. Components that render cleanly in Node.js get auto-generated HTML. Components that use DOMPurify for HTML sanitization (e.g., IconCard) also render successfully because the bundled DOMPurify detects the headless environment and falls back to a passthrough — no mock is needed. Components needing other browser APIs (`document`, `window`) fall back to curated HTML examples.

`generate-ai-manifest.js` merges three data sources: the Storybook manifest (props, types), the auto-rendered HTML, and curated data files in `scripts/ai-manifest/data/` (descriptions, CSS classes, flags, page templates). It also reads shared constants from `scripts/ai-manifest/data/constants.js` (asset URLs, required scripts, logo paths, reusable HTML snippets). When run with `--validate`, it checks for stale curated keys, accessibility anti-patterns in HTML examples, CSS class mismatches, and PropTypes coverage.

See [`scripts/ai-manifest/README.md`](https://github.com/unisdr/undrr-mangrove/blob/main/scripts/ai-manifest/README.md) for the full pipeline diagram, entry schema, and maintenance guide.
...
- **Expand auto-rendering** — six components are auto-rendered today (QuoteHighlight, StatsCard, IconCard, Pager, MegaMenu, SyndicationSearchWidget). Components that depend on `document` or `window` (e.g., ShareButtons) still need curated HTML
