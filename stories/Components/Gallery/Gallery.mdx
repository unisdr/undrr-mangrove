...
- Clickable thumbnails on left or bottom
- Keyboard navigation with arrow keys, Home, End
- Responsive layout that adapts from desktop to mobile
- Handles mixed aspect ratios (landscape, portrait, square, panoramic)
- Full ARIA support and screen reader friendly
- Optional titles and captions below content
- Configurable visibility of thumbnails, arrows, and descriptions
...
> **Implementation note (for contributors):** Image container: default 16:9 aspect ratio prevents layout shift; `object-fit: contain` preserves proportions; neutral background (`$mg-color-neutral-50`) fills empty space. Thumbnail scrolling: native overflow scrolling; auto-scrolls selected thumbnail into view; custom scrollbar styling. Navigation: arrow buttons disable at boundaries; keyboard navigation requires gallery focus; thumbnail click gives instant feedback.
...
- Use identical `id` values for different images
- Omit `alt` text
- Disable keyboard navigation without good reason
- Use extremely large images without optimization
- Mix very different image qualities
...
Gallery supports [layered hydration](https://github.com/unisdr/undrr-mangrove/blob/main/docs/HYDRATION.md). Use data attributes on a container element:

```html
<div data-mg-gallery
  data-media='[{"id":"1","src":"/photo1.jpg","alt":"Landscape","title":"Mountain View"}]'
...
|-----------|---------|-------------|
| `data-media` | `[]` | JSON array of media item objects (required). Each object with `type` of `'image'` or `'video'` (or no `type`, since `'image'` is the default) must include an `alt` property with descriptive text per WCAG 1.1.1 (Non-text Content). |
| `data-initial-index` | `0` | Index of the initially displayed item |
| `data-show-thumbnails` | `'true'` | Show thumbnail navigation (`'true'` or `'false'`) |
| `data-thumbnail-position` | `'left'` | `'left'` or `'bottom'` |
| `data-show-arrows` | `'true'` | Show navigation arrows |
| `data-arrow-style` | `'overlay'` | `'overlay'` or `'corner'` |
| `data-show-description` | `'true'` | Show title and caption below content |
| `data-enable-keyboard` | `'true'` | Enable keyboard navigation |
...
import { Gallery, fromElement } from "https://assets.undrr.org/static/mangrove/1.8.0/components/Gallery.js";
...
- **1.3** — 2026-06-19 ([mangrove#1056](https://github.com/unisdr/undrr-mangrove/pull/1056)): Consolidated `galleryAriaLabel`, `prevLabel`, `nextLabel`, `loadingLabel` props into a single `labels` object with `DEFAULT_GALLERY_LABELS` export. Backwards compatible — `DEFAULT_GALLERY_LABELS` covers all keys. Co-located `_labels.js` provides `LABELS_ES/FR/JA/ZH/AR/RU`; locale toolbar now switches translated strings in all stories.

- **1.2** — 2026-04-13: `.mg-gallery__title` now uses `$mg-font-family-headings` (Roboto Condensed). Added a `:lang(ar)` block routing to `$mg-font-family-arabic-headings`.
- **1.1** — 2026-03-04: Added hydration support with `fromElement` prop extraction and `createHydrator` integration
