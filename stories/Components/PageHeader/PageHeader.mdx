...
5. Decoration bar only (no logo, account, or language — for sub-branded sites like DELTA that provide their own navigation):
   ```jsx
...
| showLanguage| Boolean | true                                                                    | Show or hide the language switcher dropdown                                      |
...
- **1.3** — 2026-03-14: Added `showLogo`, `showAccount`, and `showLanguage` boolean props to toggle individual toolbar sections. All default to `true` for backward compatibility. Useful for sub-branded sites (e.g., DELTA) that provide their own navigation and only need the decoration stripe.
