import { Meta, Story, Canvas } from '@storybook/addon-docs/blocks';
import { Pagination } from './Pagination';
import * as PaginationStories from './Pagination.stories';

export const getCaptionForLocale = locale => {
  switch (locale) {
    case 'english':
      const engText = { text: 'Page', text2: 'of' };
      return engText;
    case 'arabic':
      const arabicText = { text: 'صفحة', text2: 'من' };
      return arabicText;
    case 'japanese':
      const japaneseText = { text: 'ページ', text2: 'の' };
      return japaneseText;
    default:
      return { text: 'Page', text2: 'of' };
  }
};

<Meta of={PaginationStories} />

> If you are creating or modifying this component, see [docs/REVIEW-CHECKLIST.md](../../../docs/REVIEW-CHECKLIST.md) for Mangrove's component standards.

# Pagination

> **Deprecated (February 2026).** Use the [Pager](?path=/docs/components-pager--docs) component instead. This component will be removed by end of 2026.

The Pagination component divides the content of the website into different pages.

### Overview

The Pagination component allows users to make large amounts of content easy to find and breaks up several entries or web content into multiple pages.

When to use:

- When web content needs to be divided into pages.

Formatting

#### Default

The Pagination component displays the number of the page and contains navigational arrows to flip through the pages. The dark colored arrow represents that there are more pages to go backward or forwards. The Light colored arrow represents the last or first page.

### Behaviors

#### States

There is only the default state of Pagination.

<Canvas of={PaginationStories.DefaultPagination} meta={PaginationStories} />

### Usage:

1. Copy the HTML from the HTML tab of canvas and add it in your html file

### Interactions

By clicking on the forward or backward arrow, the pages are flipped forwards or backwards.

## Changelog

- **1.1** — 2026-06-19 ([mangrove#1056](https://github.com/unisdr/undrr-mangrove/pull/1056)): Consolidated `prevLabel`, `nextLabel`, and `nextAriaLabel` props into a single `labels` object with `DEFAULT_PAGINATION_LABELS` export. Backwards compatible — `DEFAULT_PAGINATION_LABELS` covers all keys. Co-located `_labels.js` provides `LABELS_ES/FR/JA/ZH/AR/RU`; locale toolbar now switches translated strings in all stories.

- **Deprecated** — 2026-02: Deprecated in favor of the [Pager](?path=/docs/components-pager--docs) component. Removal planned by end of 2026. Removed from CSS bundle and webpack build.
- **1.0** — 2022-12-12: Released component
