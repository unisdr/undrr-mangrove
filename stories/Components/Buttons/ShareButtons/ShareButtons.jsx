 ...
 **/
  ...
  return (
    <div className="mg-share-buttons-wrapper" {...props}>
      <div className='mainLabel'>{labels.mainLabel}</div>
      <div>
        <button className="mg-share-button mg-button-primary" onClick={() => handleClick("Facebook")} aria-label='Share on Facebook' >
          <img src={FacebookIconWhite} alt="Facebook SVG Image" />
        </button>
        <button className="mg-share-button mg-button-primary" onClick={() => handleClick("Twitter")} aria-label='Share on Twitter' >
          <img src={TwitterIconWhite} alt="Twitter SVG Image" />

        </button>
        <button className="mg-share-button mg-button-primary" onClick={() => handleClick("LinkedIn")} aria-label='Share on LinkedIn' >
          <img src={LinkedInIconWhite} alt="LinkedIn SVG Image" />

        </button>
        <button className="mg-share-button mg-button-primary" onClick={() => handleClick("Mail")} aria-label='Share via Email' >
          <img src={MailIconWhite} alt="Mail SVG Image" />
        </button>
      </div>

      <CopyButton copiedLabel={labels.onCopy} sharedLink={sharedLink} />
    </div>
  );
}
/** 
*  CopyButton
*  @param {string} coppiedLabel - the label that will be shown when the link is coppied(should be in the right language)
*  @param {string} sharedLink - the link that will be coppied
*/
function CopyButton({ coppiedLabel, sharedLink }) {
  ...
  return (
    <button className='copyLink' onClick={() => handleCopyLink()}>
      ...
      <div className='copyText'>{coppied ? coppiedLabel : sharedLink}</div>
      <div className='copyIcon'>
