...
- [Form validation example](?path=/docs/example-form-validation--docs&args=errorDisplay:toast) — full integration demo showing Snackbar as a toast error pattern alongside inline field validation
...
- **Bug fix**: `onClose` callback is now stored in a ref so inline arrow functions no longer cause the auto-dismiss timer, keyboard listener, or focus effect to re-fire on every render.
- **Bug fix**: Close button focus now only triggers when `opened` transitions from `false` to `true`, preventing focus theft while the user interacts with other elements on the page.
