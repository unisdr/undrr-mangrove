...
- Fix: `onClose` is stored in a ref now, so passing an inline arrow function no longer re-fires the timer, keyboard listener, and focus effect on every render.
- Fix: close button only takes focus when `opened` goes from `false` to `true`, not on unrelated re-renders.
