import { Meta, Story, Canvas } from '@storybook/addon-docs/blocks';
import { Breadcrumbcomponent } from './Breadcrumbs';
import * as BreadcrumbsStories from './Breadcrumbs.stories';

export const getCaptionForLocale = locale => {
  switch (locale) {
    case 'english':
      const engText = {
        data: [
          {
            text: 'Home',
          },
          {
            text: 'Second-level',
          },
          {
            text: 'Third-level',
          },
          {
            text: 'Page title',
          },
        ],
      };
      return engText.data;
    case 'arabic':
      const arabicText = {
        data: [
          {
            text: 'الصفحة الرئيسية',
          },
          {
            text: 'المستوى الثاني',
          },
          {
            text: 'المستوى الثالث',
          },
          {
            text: 'عنوان الصفحة',
          },
        ],
      };
      return arabicText.data;
    case 'japanese':
      const japaneseText = {
        data: [
          {
            text: '家',
          },
          {
            text: 'セカンドレベル',
          },
          {
            text: '第3レベル',
          },
          {
            text: 'ページタイトル',
          },
        ],
      };
      return japaneseText.data;
    default:
      const dummy = {
        data: [
          {
            text: 'Home',
          },
          {
            text: 'Second-level',
          },
          {
            text: 'Third-level',
          },
          {
            text: 'Page title',
          },
        ],
      };
      return dummy.data;
  }
};

<Meta of={BreadcrumbsStories} />

> If you are creating or modifying this component, see [docs/REVIEW-CHECKLIST.md](../../../docs/REVIEW-CHECKLIST.md) for Mangrove's component standards.

# Breadcrumbs

The Breadcrumb provides links to each of the previously visited pages and indicates the location of the current page.

### Overview

The Breadcrumb component creates navigation links for the current pathname based on an opinionated configuration object. Drop it anywhere on the page; it requires minimal configuration to generate breadcrumbs automatically.

#### When to use:

- To indicate the location of the current page and to navigate back to each of the previously visited pages.

#### Formatting

Links are separated by a visual divider (›). The final item represents the current page and is rendered as plain text rather than a link.

#### Default

The breadcrumb appears below the main navigation bar.

### Behaviors

#### States

The component renders in a single **default** state. The current-page item receives `aria-current="page"` and is not interactive.

<Canvas of={BreadcrumbsStories.DefaultBreadcrumbs} meta={BreadcrumbsStories} />

### Usage:

1. Copy the HTML from the HTML tab of canvas.

### Interactions

- By clicking on the breadcrumb, the current page is redirected to the respective previous page.

## Changelog

- **1.2** — 2026-04-09: Fixed invalid `aria-current` value (now correctly set to `"page"` on the active item); removed redundant `aria-label` from non-active breadcrumb links
- **1.1** — 2026-03-21: Removed stale `data-viewport` animation reference from documentation.
- **1.0** — 2022-12-12: Released component
