...
The Pager supports [layered hydration](https://github.com/unisdr/undrr-mangrove/blob/main/docs/HYDRATION.md) for rendering into server-generated HTML containers.

### Data attributes

| Attribute | Type | Default | Maps to prop |
|-----------|------|---------|-------------|
| `data-page` | integer | `1` | `page` |
| `data-total-pages` | integer | `null` (mini-pager) | `totalPages` |
| `data-layout` | `'centered'` \| `'bar'` | `'centered'` | `layout` |
| `data-show-jump-to` | `'true'` \| `'false'` | `'false'` | `showJumpTo` |
| `data-aria-label` | string | `'Pagination'` | `ariaLabel` |
| `data-range-label` | string | — | `rangeLabel` |
| `data-jump-to-label` | string | — | `jumpToLabel` |
| `data-jump-to-action` | string | — | `jumpToAction` |

`onPageChange` is not extracted from the DOM — it must be provided by the consumer wrapper.
...
### Consumer wrapper (3-line pattern)

```js
import createHydrator from "@mangrove/hydrate";
import Pager, { fromElement } from "@mangrove/Pager";
createHydrator({ selector: "[data-mg-pager]", component: Pager, fromElement });
```

See the [hydration guide](https://github.com/unisdr/undrr-mangrove/blob/main/docs/HYDRATION.md) for the full API, Drupal behavior integration, and advanced patterns.
...
- **1.0** — 2026-02-13: Initial release; standalone pager extracted from SyndicationSearchWidget with bar layout, range display, jump-to-page, mini-pager mode, and empty state
