...
Renders search results as vertical cards in a responsive grid. `resultsPerPage` controls both the number of results and the grid column count.

<Canvas of={SyndicationSearchWidgetStories.CardDisplay} meta={SyndicationSearchWidgetStories} />

## Card book display

Renders search results as vertical book cards in a grid. Uses `mg-card__vc mg-card__book` classes.
