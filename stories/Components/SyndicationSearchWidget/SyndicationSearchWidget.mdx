...
> **Drupal note:** This is a complex-tier component. `fromElement` extracts only the attributes present on the container — Drupal modules and other consumer wrappers can merge additional config (`customFacets`, `customFilters`, etc.) that is too complex to express as data attributes.

## Architecture

> **Implementation note (for contributors):** This section covers internal design decisions. Consumers do not need this to integrate the widget.

The component uses React 19's concurrent rendering features for a smooth, non-blocking search experience:
