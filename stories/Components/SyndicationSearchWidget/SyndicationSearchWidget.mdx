...
### 1.1.0
- **Card display modes**: `displayMode` config option (`'list'`, `'card'`, `'card-book'`)
  - `gridColumns` config option for independent column control (2-6); falls back to `resultsPerPage`
  - Drupal image styles rewritten to match card aspect ratio (`landscape_16_9` for cards, `por` for book cards)
  - Teaser class-swapping via `swapCardVariant()` — no separate card component
- **Teaser field visibility**: `visibleTeaserFields` config option to hide specific teaser fields
  - CSS-based hiding (`display: none`) — performant, accessible, no HTML parsing
  - 7 toggleable fields: image, contentType, publicationType, date, siteName, summary, organization

### 1.0.0
- Pager styles extracted to standalone `Components/Pager` component
- Internal `Pager.jsx` is now a thin wrapper around the standalone Pager
- No API changes -- existing imports and behaviour are unchanged

### 1.0.0-alpha.2
- **Mobile filter drawer**: Left-slide drawer replaces hidden sidebar on mobile
- **Custom SelectDropdown**: Replaced Chosen.js dependency with custom component
