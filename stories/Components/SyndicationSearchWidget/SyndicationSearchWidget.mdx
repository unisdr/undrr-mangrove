...
Renders search results as vertical cards in a responsive grid. `resultsPerPage` controls both the number of results and the grid column count. Drupal image styles are automatically rewritten to `landscape_16_9` (16:9) to match the vertical card aspect ratio.
...
- **Card display modes**: `displayMode` config option (`'list'`, `'card'`, `'card-book'`)
  - Grid columns sync with `resultsPerPage`
