...
Use `iconColor` to add a circular colored background behind the icon. This pattern is used by DELTA Resilience for feature highlights.
...
| `iconColor` | String | - | Background color for circular icon badge (CSS color value). Delta mockups show 6 accent colors whose design-system semantics are not yet understood — using raw hex values until the palette is formalized. |
