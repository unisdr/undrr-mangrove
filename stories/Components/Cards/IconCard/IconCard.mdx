...
- **2.0.0** — 2026-03-29: **Breaking.** Prop renames to align with canonical card schema ([#884](https://github.com/unisdr/undrr-mangrove/issues/884)): `data` → `items`; `imgback`/`imgalt` → `image.{src,alt}`; `label` → `labels[0]`; `summaryText` → `summary`. Drupal consumers passing `data-items` JSON blobs must update the item object shape. See [migration guide](https://github.com/unisdr/undrr-mangrove/blob/main/docs/MIGRATION-SCHEMA-V2.md).
