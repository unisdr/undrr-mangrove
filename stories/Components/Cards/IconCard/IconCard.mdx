...
| `labelPosition` | String | `'content'` | Where the label renders: `'content'` (default, inside card body) or `'top'` (above the visual/image) |
...
- **1.4** — 2026-03-14: Added `borderColor` data prop for colored card borders. Added `iconFgColor` data prop for icon glyph color (distinct from `iconColor` which is the background). Icon badge border-radius is now a theme token (`$mg-card-icon-badge-radius`, default `50%`; Delta overrides to `5%`). `iconColor` now works without an `icon` class, rendering a standalone color swatch.
- **1.3** — 2026-03-14: Added `labelPosition` prop (`'content'` default or `'top'` to render label above the visual). Non-breaking — existing behavior unchanged.
