...
- **1.2** — 2026-03-14: Added `labelPosition` prop (`'content'` default or `'top'` to render label above the visual). Added `borderColor` data prop for colored card borders. Added `iconFgColor` data prop for icon glyph color (distinct from `iconColor` which is the background). Icon badge border-radius is now a theme token (`$mg-card-icon-badge-radius`, default `50%`; Delta overrides to `15%`). `iconColor` now works without an `icon` class, rendering a standalone color swatch. Added themeable card container tokens (`$mg-card-background`, `$mg-card-padding`, `$mg-card-border-radius`).
