...
- **1.1** — 2026-03-21: Renamed CSS classes to `mg-` prefix (`image-figcaption` → `mg-image-figcaption`, `image__caption` → `mg-image-caption`, `credits` → `mg-credits`, `scale-up` → `mg-scale-up`). Added BEM modifiers for size variants (`mg-image-figcaption--medium`, `mg-image-figcaption--portrait`). Replaced `defaultProps` with destructured defaults.
