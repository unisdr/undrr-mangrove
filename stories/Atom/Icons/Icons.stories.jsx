...
const Template = args => (
  <div className="icons-container">
    {args.icons.map((item, index) => {
      ...
      let classNames = 'mg-icon fa-' + item.name;
      // + (item.name.includes("animated") ? " animated" : "");
      content = <span className={classNames}></span>; // Assuming that icons use <i> for CSS-based icons
      return (
        <p key={index}>
          {content}
          &nbsp;
          <span>
            {item.label} <code>{classNames}</code>
          </span>
        </p>
      );
    })}
