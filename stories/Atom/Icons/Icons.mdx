...
**Mangrove namespace migration** - Icons now use `mg-icon-*` namespaced classes as the preferred format, with `fa-*` classes maintained for backwards compatibility.

#### New Features

- **Icon React Component** (`Icon.jsx`) - Programmatic icon rendering with automatic class normalization
  - Accepts name-only (`globe`), mg-icon-prefixed (`mg-icon-globe`), or legacy fa-prefixed (`fa-globe`) formats
  - Supports size prop: `sm`, `lg`, `xl`
- **mg-icon-* CSS classes** - All icons now have Mangrove-namespaced selectors (e.g., `.mg-icon-globe`, `.mg-icon-chart-bar`)
- **Size modifiers** - New CSS classes for icon sizing: `.mg-icon--sm`, `.mg-icon--lg`, `.mg-icon--xl`

#### Migration Notes

- New code should use `mg-icon-*` classes (e.g., `mg-icon mg-icon-globe`)
