...
13. **Never instruct an AI image model to render text or standalone icons.** Image models render text as gibberish and invent icons that drift from the OCHA + Mangrove icon systems. Recommend "no text, no rendered letters, no logos, no icons" in every prompt; if signage is conceptually needed, recommend "pictogram-only signs without rendered words." Real text and icons are added in the design tool after generation.
14. **In single-country contexts, name the kind of gathering, not just the country.** Image models default to a single ethnic phenotype when a country is named, even when the prompt asks for varied skin tones. Recommend three resolutions: (a) for local community contexts, accept ethnic homogeneity that reflects the place and vary by age / gender / ability / role instead; (b) for national-government contexts, name the within-country heritage groups explicitly (Bengali / Adivasi / Chittagong Hill Tracts; Afro-Brazilian / Indigenous / European-Brazilian; etc.); (c) for regional or international convenings, name the participating countries.
15. **Recommend specific hex codes when prompts mention colour.** "Sendai colours" alone means nothing to an image model. Always recommend including the actual hex values (#c10920 / #eb752a / #962987 / #00afae / #004f91 / #0a6969 / #591a61 / #0f78bf / #132e48 — see `AiImageryPrompts.mdx` for the full palette).
